...
import { startMdnsServer, type ServiceDiscovery } from "./mdns/index.mts";
...
if (import_meta_ponyfill(import.meta).main) {
  ...
  // 监听服务健康状态变化
  server.on('health-check', ({ serviceId, health }) => {
  ...
  // 监听服务发现事件
  server.on('service-discovery', (service) => {
  ...
  // 监听服务丢失事件
  server.on('service-lost', (service) => {
