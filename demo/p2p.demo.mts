...
async function main() {
  try {
    ...
    // 监听对等节点发现
    p2pNode.on('service-discovery', (service: ServiceDiscovery) => {
      const discoveredPeerId = service.metadata?.peerId || service.serviceName;
      console.log('\n发现新对等节点:', {
    ...
    // 监听对等节点离线
    p2pNode.on('service-lost', (service: ServiceDiscovery) => {
      if (service.metadata?.peerId) {
        peers.delete(service.metadata.peerId);
        console.log('\n对等节点离线:', {
